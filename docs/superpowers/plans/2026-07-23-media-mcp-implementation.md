...
- Behavior: fetch latest inbound media messages → skip already-processed ids → for each attachment (max 3 total): download → sniff → skip modality if tenant toggle off (note in output) → provider.describe → assemble labeled sections (`🎤 Voice note transcript: …` / `📷 Image: …` / `📄 Document: …`). Failures per attachment become `[attachment could not be read: <reason>]` — the function NEVER throws for per-attachment errors. If no unprocessed media found → text `"[no new attachments found]"`. Marks message ids processed ONLY after assembly. Records `tool_call` event with counts (no content).
...
- Cycle: list newest 25 conversations → those with `updatedAt > cursor` (first run: only prime the cursor, wake nothing — avoids storming old conversations on deploy) → per conversation list messages → signature matches not in processed → mark processed with `waker:` prefix (`waker:<msgId>` — independent of tool-leg dedupe) → ONE `chatCompletion` per conversation (not per message), `assistant_id` = conversation's assistant if present else tenant default. Instruction text exactly:
